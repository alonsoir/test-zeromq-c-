// External Traffic Classification Model Implementation
...
TrafficDetector::Prediction TrafficDetector::predict(const Features& features) const noexcept {
    ...
    float internet_votes = 0.0f;
    float internal_votes = 0.0f;
    ...
    for (const auto* tree : trees) {
        ...
        internet_votes += leaf.value[0];
        internal_votes += leaf.value[1];
    }

    // Average probabilities across all trees
    const float internet_prob = internet_votes / 100.0f;
    const float internal_prob = internal_votes / 100.0f;

    // Determine predicted class
    const int class_id = (internal_prob > internet_prob) ? 1 : 0;
    const float probability = (class_id == 0) ? internet_prob : internal_prob;
