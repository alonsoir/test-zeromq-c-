# ml-training-scripts/external_traffic/GenerateTrafficCPPForest.py
import json
import pickle
import sys
...
def load_traffic_model(model_path: str, dataset_path: str):
    """Carga el modelo de tráfico, scaler y metadata"""
    ...
    try:
        ...
        print(f"✅ Traffic Scaler loaded: {scaler_path}")
    ...
    print(f"✅ Traffic Model loaded: {model.n_estimators} trees, {dataset_info['model_info']['n_features']} features")
...
def extract_tree_structure(forest_model, feature_names):
    ...
    print("🌳 Extracting traffic tree structures...")
    ...
    for i, estimator in enumerate(forest_model.estimators_):
        ...
        tree_data = {
            ...
            'value': tree.value.tolist()  # Probabilidades por clase [internet, internal]
        }
...
def generate_traffic_cpp_header(forest_data, dataset_info, output_path: str):
    """Genera el header C++20 para el modelo de tráfico con predict() COMPLETO"""
...
inline constexpr TrafficNode {tree_key}[] = {{
"""
        ...
        for j in range(tree_data['n_nodes']):
            ...
            value_0 = tree_data['value'][j][0][0]  # P(internet)
            value_1 = tree_data['value'][j][0][1]  # P(internal)
            ...
            if feature_idx >= 0:
                ...
                trees_definitions += f"  // Leaf: P(internal)={value_1:.4f}"
    ...
    header_content = f"""// AUTO-GENERATED Traffic Classification Trees
...
inline constexpr size_t TRAFFIC_NUM_TREES = {n_trees};
inline constexpr size_t TRAFFIC_NUM_FEATURES = {len(feature_names)};
...
"""
    ...
    print(f"✅ Generated COMPLETE Traffic header: {output_path}")
...
def main():
    ...
    model, dataset_info, scaler = load_traffic_model(
        input_pkl,
        "traffic_classification_dataset.json"
    )
