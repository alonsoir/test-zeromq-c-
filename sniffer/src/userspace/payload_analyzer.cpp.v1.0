...
bool PayloadAnalyzer::check_pe_header(const uint8_t* data, uint16_t len) {
    ...
    if (pe_offset > len - 4 || pe_offset > 1024) {
        return false;
    }
...
float PayloadAnalyzer::calculate_entropy_fast(const uint8_t* data, uint16_t len) {
    ...
    // Calculate Shannon entropy using pre-computed log2 table
    float entropy = 0.0f;
    const float len_f = static_cast<float>(len);
    
    for (uint32_t freq : byte_freq_) {
        if (freq > 0) {
            float probability = static_cast<float>(freq) / len_f;
            // H = -Σ(p * log2(p))
            // Using lookup table: log2(freq) - log2(len)
            entropy -= probability * (log2_table_[freq] - log2_table_[len]);
        }
    }
